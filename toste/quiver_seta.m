...
cam = pwd;
cd('/usr/local/MATLAB/R2013a/toolbox/matlab/specgraph/')
%QUIVER Quiver plot.
...
%   $Revision: 5.21.6.24 $  $Date: 2011/07/25 03:49:36 $
...
while ischar(args{nin}),
    ...
    if ~isempty(vv) && strcmpi(vv(1),'f')
        ...
        if ~isempty(msg),
            error(message('MATLAB:quiver:UnknownOption', vv));
        end
...
%             msg.identifier = 'MATLAB:quiver:UVSizeMismatch';
