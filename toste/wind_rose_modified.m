function varargout = wind_rose_modified(D,F,varargin)
...
for i=1:length(Ay)-1
  ...
  if ~isempty(b)
  medias.velo_med(i) = mean(b);
  medias.velo_max(i) = max(b);
  end
...
for i=1:length(Ay)-1
  if n(i)
    ...
    for j=1:length(Ag)-1
      ...
      if E(i,j)>0, handles(end+1)=fill(x,y,cor{jcor},'EdgeColor','none'); end
      r1=r2;
    end
  end
end
...
h(1)=text(-g-ri, 0,'W', 'VerticalAlignment','top',   'HorizontalAlignment','left', args{:});
h(2)=text( g+ri, 0,'E', 'VerticalAlignment','top',   'HorizontalAlignment','right',args{:});
h(3)=text( 0,-g-ri,'S','VerticalAlignment','bottom','HorizontalAlignment','left', args{:});
h(4)=text( 0, g+ri,'N','VerticalAlignment','top',   'HorizontalAlignment','left', args{:});
handles=[handles h];
...
if legType==1 % contimuous.
  for j=1:length(Ag)-1
    ...
    y1=y0+h;
    handles(end+1)=fill([x0 x1 x1 x0],[y0 y0 y1 y1],cor{j},'EdgeColor','none');
...
elseif legType==2 % separated boxes.
  for j=1:length(Ag)-1
    ...
    y1=y0+h;
    handles(end+1)=fill([x0 x1 x1 x0],[y0+dy y0+dy y1 y1],cor{j},'EdgeColor','none');
